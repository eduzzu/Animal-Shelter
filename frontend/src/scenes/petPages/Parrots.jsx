...
const Parrots = () => {
    ...
    const breeds = Array.from(new Set(parrots.map((p) => p.breed))).sort();
    const ages = Array.from(new Set(parrots.map((p) => p.age))).sort();
    const genders = Array.from(new Set(parrots.map((p) => p.gender)));
    const sizes = Array.from(new Set(parrots.map((p) => p.size))).sort().reverse();
