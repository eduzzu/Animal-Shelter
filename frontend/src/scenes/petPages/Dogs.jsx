...
const Dogs = () => {
    ...
    const getDogs = async () => {
        try {
            ...
            const dogsFiltered = data.filter(dog => dog.category === "Dog");
    ...
    const filterDogs = dogs.filter((dog) => {
        return (
            (!selectedBreed || dog.breed === selectedBreed.value) &&
            (!selectedAge || dog.age === selectedAge.value) &&
            (!selectedGender || dog.gender === selectedGender.value) &&
            (!selectedSize || dog.size === selectedSize.value)
        );
    });

    const displayDogs = filterDogs.length > 0 ? filterDogs : 0;
    ...
    return (
        ...
        displayDogs.map(({ _id, picturePath, name, age, category, breed }) => (
